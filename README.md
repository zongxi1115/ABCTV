...
| MAX_DEVICE                        | 单账号允许最大同时在线设备数（超出踢出旧设备）；可在后台【站点配置】里修改并持久化（环境变量作为默认值） | 0-50（0=不限制）                 | 0                                                                                                                          |
| ADMIN_CONTROL                     | 注册控制开关：true=禁用注册，仅允许登录                                                                  | true / false                     | false                                                                                                                      |
| ADMIN_USERNAME                    | 额外后台管理员用户名（可选）                                                                             | 任意字符串                       | （空）                                                                                                                     |
| ADMIN_PASSWORD                    | 额外后台管理员密码（可选）                                                                               | 任意字符串                       | （空）                                                                                                                     |
| UPSTASH_URL / KV_REST_API_URL     | HTTP 类型的 Upstash KV API URL                                                                           | 任意 URL                         | （空）                                                                                                                     |
| UPSTASH_TOKEN / KV_REST_API_TOKEN | HTTP 类型的 Upstash KV API Token                                                                         | 任意字符串                       | （空）                                                                                                                     |
