...
function extractPartialReply(jsonString: string): string {
  // 1. 如果整体可以直接解析为 JSON 对象且包含 valid reply
  try {
  ...
  // 2. 尝试正则提取 "reply": "..."
  // 匹配模式： "reply"\s*:\s*" (内容...) (结束引号或字符串结束)
  // 注意 JSON 字符串中可能含有转义引号 \"
  ...
  if (match) {
    ...
    try {
      ...
      // 如果解析失败（通过 JSON.parse），很可能是因为截断导致转义符不完整等
      // 降级处理：手动替换常见转义符
      return rawContent
        .replace(/\\n/g, '\n')
        .replace(/\\r/g, '\r')
    ...
    try {
      ...
      const handleEvent = (event: string, dataText: string) => {
        ...
        if (event === 'delta') {
          ...
          if (!text) return;
          preview += text;
          const partialReply = extractPartialReply(preview);
          updateAssistant({ content: partialReply || '正在思考...' });
          return;
        }
        ...
        if (event === 'status') {
          const stage =
            data && typeof data.stage === 'string' ? data.stage : 'thinking';
          if (stage === 'search') {
            const q = data && typeof data.query === 'string' ? data.query : '';
            updateAssistant({ content: `正在站内搜索：${q}…` });
          }
          return;
        }
        ...
        if (event === 'final') {
          ...
          updateAssistant({ content: reply, items, raw, isStreaming: false });
          return;
        }
            ...
            <div className='flex-1  p-4 space-y-4'>
              {messages.length === 0 ? (
                ...
                messages.map((m) => (
                  <div
                    key={m.id}
                    className={`rounded-xl px-3 py-2 text-sm whitespace-pre-wrap w-fit ${
                      m.role === 'user'
                  ...
                  >
                    ...
                    {m.role === 'assistant' &&
                      ...
                      m.items.length > 0 && (
                        <div className='mt-3'>
                          {/* Desktop View: Inline links with hover card */}
                            ...
                            {m.items.map((it, idx) => (
                              ...
                              >
                                ...
                                <div className='absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-72 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 pointer-events-none'>
                                  <div className='bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-black/10 dark:border-white/10 p-4 text-sm'>
                          ...
                          {/* Mobile View: Vertical list with details */}
